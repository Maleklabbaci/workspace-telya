...
const TaskCard: React.FC<TaskCardProps> = ({ task, projectMembers, onAssignTask, onSelectTask }) => {
  return (
    <button
      ...
      className="w-full text-left rounded-2xl hover:-translate-y-1 transition-transform duration-200 focus:outline-none focus:ring-2 focus:ring-ring"
    >
      <Card className="p-4">
        <h4 className="font-bold text-card-foreground mb-2">{task.title}</h4>
        {task.description && (
